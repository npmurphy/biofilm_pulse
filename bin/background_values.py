...
channels = {
    ...
    "green_raw": ["actual"],
}  # get the raw YFP signal with no subtractions
file_keys = {
    "bg": "WT|RFP",
    "autofluor": "WT",
    "bleedthrough": "RFP",
...
def compute_subtraction(row, maskname, color, subtraction_value, section=""):
    ...
    img = lib.util.array_sub(img, subtraction_value)
...
def get_new_bg_flour_func(pa):
    ...
    for (
        color
    ) in channels.keys():  # yes I know, we read each mask twice but we just run once.
        print("Color ", color)
        ...
        for subtraction in channels[color]:
            ...
            subtraction_value = sum(
                [means[color + "_" + s] for s in acumulated_subtraction_list]
            )

            def file_process(row, section=""):
                if re.match(file_keys[subtraction], row["strain"]):
                    return compute_subtraction(
                        row,
                        mask_keys[subtraction],
                        color,
                        subtraction_value,
                        section=section,
                    )
            ...
            if subtraction == "actual":
                ...
                fileData[color + "_" + subtraction] = fileData.apply(
                    file_process, axis=1
                )
            acumulated_subtraction_list += [subtraction]

    # fileData = fileData.drop(columns=[c + "_none" for c in channels])
    fileData = fileData.drop([c + "_none" for c in channels], axis=1)
