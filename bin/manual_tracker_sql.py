...
class State:
    def __init__(
        ...
        if self.compileddata is not None:
            self.compileddata["gr"] = (
                self.compileddata["green"] / self.compileddata["red"]
            )
        ...
        self.gridspec = gridspec.GridSpec(
            4, 2, height_ratios=[1, 0.1, 0.1, 0.1], width_ratios=[0.6, 0.4]
        )
        ...
        if len(self.bg_img.shape) == 2:
            ...
            self.art_img = self.ax_img.imshow(
                self.bg_img, vmin=0, interpolation="nearest"
            )
        ...
        self.ui_selectors.append(
            self.fig.canvas.mpl_connect(
                "button_press_event", self.select_cell_id_from_tree
            )
        )
        ...
        self.ui_selectors.append(
            self.fig.canvas.mpl_connect("button_press_event", self.select_cell)
        )
        self.ui_selectors.append(
            self.fig.canvas.mpl_connect("button_press_event", self.select_frame)
        )

        self.ax_cell_trace = plt.subplot(
            self.gridspec[3, 0:2], sharex=self.ax_cell_compiled_trace
        )
        self.comp_bar = None
        self.trace_bar = None
        self.cell_trace_plots = []
        self.cur_image_art = self.ax_cell_trace.axvspan(
            self.current_image - 0.5, self.current_image + 0.5, color="grey", alpha=0.4
        )
    ...
    def bit16_to_bit8(self, im):
        ...
        imr = skimage.exposure.rescale_intensity(
            modify(im[:, :, 0]), in_range=(0, red_max), out_range=(0, 255)
        ).astype(np.uint8)
        img = skimage.exposure.rescale_intensity(
            modify(im[:, :, 1]), in_range=(0, grn_max), out_range=(0, 255)
        ).astype(np.uint8)
        imb = skimage.exposure.rescale_intensity(
            modify(im[:, :, 2]), in_range=(0, 6897), out_range=(0, 255)
        ).astype(np.uint8)
    ...
    def plot_cell_compiled_trace(self, frame, cell):
        ...
        self.comp_bar = self.ax_cell_compiled_trace.axvspan(
            frame - 0.5, frame + 0.5, color="grey", alpha=0.4
        )
        ...
        if self.compileddata is None:
            ...
            for i, (ch, cl) in enumerate(plots):
                ...
                lin_smooth = scipy.ndimage.gaussian_filter1d(
                    lin_data, sigma=5, mode="mirror"
                )
                ...
                if (ch not in self.compiled_plots.keys()) or self.compiled_plots[
                    ch
                ] is None:
                    lin_line, = self.ax_cell_compiled_trace.plot(
                        lin_frames, lin_smooth, color=cl, linestyle=":"
                    )
    ...
    def plot_cell_path(self, frame, cell):
        ...
        self.trace_bar = self.ax_cell_trace.axvspan(
            frame - 0.5, frame + 0.5, color="grey", alpha=0.4
        )
    ...
    def select_frame(self, event):
        ...
        if event.inaxes is None:
            return None
        if (event.inaxes.name == "cell_trace") or (
            event.inaxes.name == "compiled_trace"
        ):
    ...
    def make_current_cell_like_previous_frame(self):
        ...
        if self.trackdata.get_cell_state(prev_frame, self.current_cell_id) != 0:
            self.trackdata.copy_cell_info_from_frame(
                self.current_cell_id, prev_frame, self.current_image
            )
    ...
    def update_track_data(self):
        ...
        if self.interactive_cell is not None:
            ...
            self.trackdata.set_cell_properties(
                self.current_image, self.current_cell_id, properties
            )
            print(
                "Saved",
                self.trackdata.get_cell_params(
    ...
    def track_cell(self):
        ...
        print(
            ...
            self.trackdata.get_cell_properties(
                self.current_image, self.current_cell_id
            ),
        )
        print(
            ...
            self.trackdata.get_cell_properties(
                self.current_image + 1, self.current_cell_id
            ),
        )
        ntd = cell_tracker.gui_interpolate(
            self.trackdata, self.current_cell_id, self.current_image
        )
        self.trackdata = ntd
        print("tracked")
        print(
    ...
    def on_key_press(self, event):
        event_dict = {
            # "t" : self.track_cell,
            "t": lambda: self.approve_track_status(
                self.current_cell_id, self.current_image
            ),
            "i": lambda: self.set_schntiz_cell_id(
                self.current_cell_id, self.current_image
            ),
