...
class State():
    ...
    def __init__(self, filepattern, track_path, compiled_path, image_start=1, cell_start:int=1, 
    ...
    def add_new_cell_to_frame(self, cell_id):
        ...
        if self.trackdata.get_cell_state(self.current_image, cell_id) != 0:
            ...
            xpos = x0 + (x1 - x0)/2
            ypos = y1 + (y0 - y1)/2
            temp_properties = {"row": ypos, "col":xpos, "length": 22, "width": 6, "angle": 0, "state":"there"}
    ...
    def set_cell_state(self, state):
        ...
        self.trackdata.set_cell_state(self.current_image, self.current_cell_id, state_name)
