...
biofilm_images = [
    (
        "wt_sigar_sigby",
        "SigB_48hrs_center_1_1_100615_sect.tiff",
        ((800, 1300), (300, 1600)),
    ),
    (
        "delru_sigar_sigby",
        "delRU_48hrs_3_6_100615sect.tiff",
        ((1080, 1580), (500, 1800)),
    ),
    (
        "delqp_sigar_sigby",
        "delQP_48hrs_2_5_100615_sect.tiff",
        ((900, 1400), (150, 1450)),
    ),
]
...
for i, (strain, imgpath, slice_it) in enumerate(biofilm_images):
    ...
    imr = skimage.exposure.rescale_intensity(
        im[:, :, 0], in_range=(1500, 18000), out_range=(0, 255)
    ).astype(np.uint8)
    img = skimage.exposure.rescale_intensity(
        im[:, :, 1], in_range=(0, 6000), out_range=(0, 255)
    ).astype(np.uint8)
