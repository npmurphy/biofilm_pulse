...
def using_timelapse_movie():
    image_path = (
        "proc_data/iphox_movies/BF10_timelapse/Column_2/Column_2_t{0:03}_ch{1}.tif"
    )
    ...
    yfp_images = [
        skimage.io.imread(image_path.format(i, "01")) for i in frame_selection
    ]
    rfp_images = [
        skimage.io.imread(image_path.format(i, "00")) for i in frame_selection
    ]
    ...
    color_indv = [
        np.dstack([r, y, np.zeros_like(r)]) for y, r in zip(proc_yfp, proc_rfp)
    ]

    color_anotate = [
        add_annotations(im, i, start_time, step_time)
        for i, im in zip(frame_selection, color_indv)
    ]
...
def using_snap_shots():
    ...
    base_path = os.path.join(
        thisdir, "../../proc_data/iphox_live_gradient_checks/BF_12hoursnaps2/"
    )
    ...
    # arrow_locations = {} # in XY
    yfp_images = [
        skimage.io.imread(image_path.format(column, t, "g")) for t in time_selection
    ]
    rfp_images = [
        skimage.io.imread(image_path.format(column, t, "r")) for t in time_selection
    ]
    print(yfp_images[0].shape, rfp_images[0].shape)
    # max_yfp = 4000 #350 #np.max([ y.max() for y in yfp_images])
    ...
    proc_yfp = [
        process_fp(im, roi_time(t), min_yfp, max_yfp)
        for (t, im) in zip(time_selection, yfp_images)
    ]
    proc_rfp = [
        process_fp(im, roi_time(t), min_rfp, max_rfp)
        for (t, im) in zip(time_selection, rfp_images)
    ]
    color_indv = [
        np.dstack([r, y, np.zeros_like(r)]) for y, r in zip(proc_yfp, proc_rfp)
    ]
    ...
    color_anotate = [
        add_annotations(im, i * 60) for i, im in zip(time_selection, color_indv)
    ]
    ...
    length = 20
    final = lib.figure_util.draw_scale_bar(
        final,
        150,
        20,
