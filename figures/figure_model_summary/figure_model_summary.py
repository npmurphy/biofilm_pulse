...
trace_wt_pulse = glob(
    runf + "bfsim_b_trace_seed16222|*pscale_a=0.7*,pscale_b=0.25*.tsv"
)[0]

pulse_wt_df = simulation_processor.get_dataset(
    pulse_wt_info, max_distance=140.0, spore_time_hours=0.5
)
pulse_2x_df = simulation_processor.get_dataset(
    pulse_2x_info, max_distance=140.0, spore_time_hours=0.5
)
...
traces[["Time", "A", "B"]].to_csv("source_data/figure6_b.tsv", sep="\t")
...
sigb_pulse_ax, sb_p_plots = subfig_sig_pulse_gradient.get_figure(
    sigb_pulse_ax, pulse_wt_df, pulse_2x_df
)
...
spor_pulse_ax, sp_p_plots = subfig_spore_gradient.get_figure(
    spor_pulse_ax, pulse_wt_df, pulse_2x_df
)
...
spor_pulse_ax.text(
    0.5,
    0.005,
    "Distance top of biofilm (simulated)",
...
fig.subplots_adjust(
    left=0.10, right=0.98, top=0.98, bottom=0.11, hspace=0.26, wspace=0.25
)
