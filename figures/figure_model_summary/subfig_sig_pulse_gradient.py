...
def get_figure(ax, wt_df, x2_df, **kwargs):
    ax, wtp = plot_sigb(ax, wt_df, color=figure_util.strain_color["JLB077"], label="$s_B$")
    ax, x2p = plot_sigb(ax, x2_df, color=figure_util.strain_color["JLB117"], label="2 $\\times s_B$")
