...
def get_into_plotform(df, params, max_distance, spore_time_hours):
    ...
    df["resp_a"] = (
        params["pscale_a"] * df["stress"] * params["b0"]
    )  # + params["ascale_a"]
    df["resp_b"] = (
        params["pscale_b"] * df["stress"] * params["b0"]
    )  # + params["ascale_b"]
...
def get_dataset(filep, max_distance, spore_time_hours):
    ...
    biofilm_df = get_into_plotform(
        biofilm_df, filename_parms, max_distance, spore_time_hours
    )
    return biofilm_df
