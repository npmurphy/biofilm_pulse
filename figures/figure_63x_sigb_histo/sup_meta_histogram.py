...
def main():
    curve_score_methods = {
        "std": ("Standard deviation", (0.0, 1.0), lambda d, h, b: np.std(d)),  # 1.5,
        ...
        "kurtosis": ("Kurtosis", (0.0, 8.0), lambda d, h, b: scipy.stats.kurtosis(d)),
    }
    ...
    width, height = figure_util.get_figsize(
        figure_util.fig_width_medium_pt, wf=1.0, hf=0.6
    )
    fig.subplots_adjust(
        left=0.085, right=0.89, top=0.89, bottom=0.13, hspace=0.20, wspace=0.25
    )
